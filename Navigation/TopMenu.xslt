...
<xsl:stylesheet version="1.0"  xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
  ...
  <xsl:template match="//Page">
    <xsl:if test="@Allowclick='True' and not(count(Page)) and @Sort>'1'">
      <li>
        <xsl:attribute name="class">
          <xsl:text disable-output-escaping="yes">menu-tools__item dw-mod</xsl:text>
          <xsl:if test="@InPath='True' or @Active='True'"> menu__item--active </xsl:if>
        </xsl:attribute>
        <a>
          <xsl:attribute name="class">
            <xsl:text disable-output-escaping="yes">menu-tools__link</xsl:text>
            <xsl:if test="@InPath='True' or @Active='True'"> menu-tools__link--active </xsl:if>
